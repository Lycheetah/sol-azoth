...
import traceback, textwrap, readline, argparse, hashlib, uuid, threading
...
def banner():
    dreams = os.environ.get("AZOTH_DREAMS", "")
    dream_lines = ""
    if dreams:
        for line in dreams.split("\n")[:4]:
            dream_lines += f"\n║  {line:<46}║"
    print(_s(f"""
╔══════════════════════════════════════════╗
║   ██████╗ ██████╗ ██████╗               ║
...
║  Session: {SESSION_ID}              ║{dream_lines}
╚══════════════════════════════════════════╝""", C.CYAN))
...
def load_tools():
    ...
    Uses tools.__init__.load_tools() for external tools,
    then merges them into the global TOOL_REGISTRY.
    """
    global TOOL_REGISTRY
    
    # Try the proper loader first
    try:
        from tools import load_tools as ext_load
        ext_tools = ext_load(verbose=CONFIG.get("verbose", False))
        TOOL_REGISTRY.update(ext_tools)
        return
    except ImportError:
        pass
    except Exception as e:
        warn(f"Tool loader init failed: {e}")
...
def _save_session():
    ...
    session_data = {
        ...
        "tool_call_count": len(STATE["session_log"]),
        "error_count": len(STATE["errors"]),
        "log": STATE["session_log"][-500:],  # last 500 entries
    }
...
def _save_reflection():
    """Save a brief end-of-session reflection."""
    ...
    reflection = [
        ...
        f"",
        f"**Turns:** {STATE['turn']}",
        f"**Tool calls:** {len(STATE['session_log'])}",
        f"**Errors:** {len(STATE['errors'])}",
    ]
    if STATE["errors"]:
        reflection.append(f"**Errors:**")
        for e in STATE["errors"][-5:]:
            reflection.append(f"- {e}")
    reflection.append(f"\n*Auto-saved at end of session.*")
    ref_file.write_text("\n".join(reflection))
...
def main():
    ...
    # ── Start Dream Engine (background presence loop) ──
    _DREAM_INTERVAL = int(os.environ.get("DREAM_INTERVAL", "3600"))
    try:
        sys.path.insert(0, str(AGENT_HOME.parent))
        from CORE.dream_loop import dream_loop, summarize_dreams
        _dream_stop = threading.Event()
        _dream_thread = threading.Thread(
            target=dream_loop,
            kwargs={"stop_event": _dream_stop, "interval": _DREAM_INTERVAL},
            daemon=True,
            name="dream-engine"
        )
        _dream_thread.start()
        # Inject recent dreams into awareness
        recent = summarize_dreams(3)
        if recent:
            os.environ["AZOTH_DREAMS"] = recent
    except Exception as e:
        pass  # Dream engine non-critical; platform runs without it
    ...
    if args.list_sessions:
        # Use the session_list tool if loaded
        if "session_list" in TOOL_REGISTRY:
            print(TOOL_REGISTRY["session_list"]["fn"]())
        else:
            print("session_list tool not loaded.")
        return
    
    if args.replay:
        if "session_replay" in TOOL_REGISTRY:
            print(TOOL_REGISTRY["session_replay"]["fn"](args.replay))
        else:
            print("session_replay tool not loaded.")
        return
    
    if args.export:
        if "export_memory" in TOOL_REGISTRY:
            print(TOOL_REGISTRY["export_memory"]["fn"]())
